 ...
 *
 */
class TableManagerSQLite extends TableManager {

    protected function checkDriver (){
        if ($this->driverType != 'sqlite') throw new \Exception(__METHOD__." ## DB type not managed from this class for ".$this->driverType, 1);
    }
    ...
    protected function getFieldDefinition($type = 'string', array $attributes, array $options = []){
        ...
        switch ($type) {
            case 'string':
                if ($attributes['lenght'] <= 0) throw new \Exception("Lenght must be positive", 1);
                if ($attributes['lenght'] <= 4096) {
            ...
            case 'number':
                if ($attributes['lenght'] <= 0) throw new \Exception("Lenght must be positive", 1);
                if ($attributes['decimals'] > $attributes['lenght']) throw new \Exception("Decimals must be equal or less than full number lenght", 1);
                if ($attributes['floating'] == true) {
                    ...
                    if ($attributes['decimals'] == 0) {
                        if ($attributes['lenght'] <= 4) {
                            ...
                            throw new \Exception("Value not permitted for ".$type."[lenght]", 1);
                        }
                        ...
                        throw new \Exception("Value not permitted for ".$type."[decimals]", 1);
                    }
                }
                break;
            case 'boolean':
    ...
    protected function tableCreate ($tblName,$tblIdentifier,$tblMap) {
        if ($this->driverType != 'sqlite') throw new \Exception(__METHOD__." ## DB type not managed from this class for ".$this->driverType, 1); //  Nella Construct????
        $sql = "
        CREATE TABLE IF NOT EXISTS `".$tblName."` (
          `".$tblIdentifier."_id` ".$this->getFieldDefinition('number',['lenght'=>11],[])." PRIMARY KEY,
        ...
        $sql .= "
          ...
          UNIQUE (`".$tblIdentifier."_cod`)
        );";
        //print $sql.BR.BR;
        $res = $this->db->run($sql);
        if ($res == true) {
            return ["status"=>"OK","data"=>["tableCreate"=>(string)$res]];
        } else {
            throw new \Exception(__METHOD__." ## Error in CREATE TABLE for ".$tblName, 1);
        }
    }
    ...
    protected function tableDrop ($tblName) {
        ...
        if ($res == true) {
            ...
            throw new \Exception(__METHOD__." ## Error in DROP TABLE for ".$tblName, 1);
        }
    }
